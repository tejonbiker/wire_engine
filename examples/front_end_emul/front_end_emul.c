...
int PID(FiguraInfo *fig1, FiguraInfo *fig2, FiguraInfo *dest,ErrorContenedor *cont, float *totalError)
{
    ...
    *totalError=0;
    ...
    ptrErrorAcumulativo=cont->ErrorAcumulativo;
	...
	ptrErrorAcumulativo[0]+=ptrErrorActual[0];
...
int ErrorCreaDesde(ErrorContenedor *cont,FiguraInfo *fig)
{
    ...
    cont->ErrorAcumulativo=(float*)malloc(sizeof(float)*3*fig->nVertices);
...
int ErrorLimpia(ErrorContenedor *cont)
{
    ...
    memset(cont->ErrorAcumulativo,0,sizeof(float)*3*cont->longitud);
